        ...
        <div className='mt-10 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl mx-auto'>
          {loading ? (
            [...Array(4)].map((_, i) => (
              <div
                key={i}
                className='animate-pulse bg-gray-100 border border-gray-200 rounded-xl p-4 h-[76px]'
              />
            ))
          ) : error ? (
            <div className='col-span-full text-sm text-red-600 font-medium'>{error}</div>
          ) : (
            stats.map((stat) => (
              <div
                key={stat.name}
                className='bg-white border border-gray-200 border-t-2 border-t-brand-gold rounded-xl p-4 text-left shadow-sm'
              >
